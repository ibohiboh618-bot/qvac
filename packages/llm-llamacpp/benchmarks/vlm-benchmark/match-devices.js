...
function parseArgs (argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === '--shipping') bucket = 'shipping'
      ...
      out.outFile = argv[++i]; bucket = null
    } else if (a === '--label') {
      const [f, n] = String(argv[++i]).split('='); out.names[f] = n
    } else if (bucket) out[bucket].push(a)
  }
  return out
}

// Per-run cell stats. Peak RSS comes from the per-row rss_mb field (harness peakRssMb,
// already in MB; max across measured blocks) — the same source aggregate.js reports,
// so this stays aligned with main's memory reporting (no separate [VLMMEM] marker).
function runStats (file, build, name) {
  const { rows } = parseLog([{ label: '', file }])
  const byCell = {}
  ...
  function cellStat (cell) {
    ...
    const rssVals = ok.map(r => r.rss_mb).filter(v => v != null)
    return {
      n: ok.length,
      ...
      rssMb: rssVals.length ? Math.max(...rssVals) : null
    }
  }
