...
> ## ▶ THIS BRANCH — QVAC-20557 Bug-1 (SVE/armv9 CPU 12 kHz tone) fix verify
> The ~12 kHz tone in **Findings** below is now **root-caused** to the `ggml_vec_dot_f32` SVE leftover-tail: `svmad_f32_m`
> drops the main-loop partial sums on inactive lanes → biased f32 dot → a Nyquist comb out of the HiFT/ISTFT
> `conv_transpose_1d`. This branch pins a **`ports/ggml-speech` overlay** = the fix (`svmad_f32_m → svmla_f32_m`, PR #30)
> + a `TTS_SVE_DOT_UNFIXED` A/B toggle + a once-per-process `[sve-diag]` print. **Run `01` → `02` → `07-run-bug1-sve.sh
