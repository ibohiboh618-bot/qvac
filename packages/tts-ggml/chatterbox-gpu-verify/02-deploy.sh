...
for f in chatterbox-t3-turbo.gguf chatterbox-s3gen.gguf chatterbox-t3-mtl.gguf chatterbox-s3gen-mtl.gguf; do
  if [ -f "$MODELS_DIR/$f" ]; then
    "${ADB[@]}" push "$MODELS_DIR/$f" "$DEVICE_DIR/models/"
  else
    echo "   WARN missing $MODELS_DIR/$f (turbo needs the first two; mtl needs the last two)"
  fi
done
