...
# Both engines: Supertonic (supertonic2.gguf) + Chatterbox (t3-turbo + s3gen).
MODEL_GROUP="${MODEL_GROUP:-supertonic2,chatterbox}"
FORCE=0
[ "${1:-}" = "--force" ] && FORCE=1
...
# ── 3. models from the public registry (Supertonic + Chatterbox) ─────────────
# All three ggufs are needed: supertonic2.gguf, chatterbox-t3-turbo.gguf,
# chatterbox-s3gen.gguf. Re-download if any is missing.
NEED_MODELS=0
for g in supertonic2.gguf chatterbox-t3-turbo.gguf chatterbox-s3gen.gguf; do
  [ -f "$BUNDLE_DIR/models/$g" ] || NEED_MODELS=1
done
if [ "$FORCE" = "1" ] || [ "$NEED_MODELS" = "1" ]; then
  say "downloading model groups '$MODEL_GROUP' from the public QVAC registry"
  mkdir -p "$BUNDLE_DIR/models"
  ( cd "$BUNDLE_DIR" && node download-tts-ggml-models.js --group "$MODEL_GROUP" --output "$BUNDLE_DIR/models" ) \
    || { miss "model download failed — needs internet; or ask the colleague for the .gguf files"; exit 1; }
...
chk_f "$BUNDLE_DIR/jfk.wav"          "jfk.wav (Chatterbox ref)" "shipped in the bundle — ask the colleague if absent"
chk_f "$BUNDLE_DIR/node_modules.tgz" "node_modules.tgz"    "re-run setup.sh (npm install + tar)"
chk_f "$BUNDLE_DIR/models/supertonic2.gguf"         "supertonic2.gguf"         "re-run setup.sh, or ask the colleague"
chk_f "$BUNDLE_DIR/models/chatterbox-t3-turbo.gguf" "chatterbox-t3-turbo.gguf" "re-run setup.sh, or ask the colleague"
chk_f "$BUNDLE_DIR/models/chatterbox-s3gen.gguf"    "chatterbox-s3gen.gguf"    "re-run setup.sh, or ask the colleague"
chk_g "$BUNDLE_DIR/prebuilds/android-arm64/*.bare" "prebuild (.bare)" "shipped in the bundle — ask the colleague"

echo
if [ "$READY" = "1" ]; then
  ok "READY — connect the phone and run:  bash run-on-device.sh"
else
