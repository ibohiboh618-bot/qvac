...
// GPU-vs-CPU output correlation gate (QVAC-20557 Mali GPU correctness diagnostic).
//
...
// end-to-end corr is NOT a valid GPU-vs-CPU gate. For Chatterbox use audio
// sanity (finite + audible) + the native `[gpu-diag]` per-stage trace instead.
...
// Assert GPU output correlates with CPU (for DETERMINISTIC engines only).
...
function assertSampleCorrelation (t, engineTag, gpu, cpu, opts = {}) {
  ...
  // Undefined correlation (silent / NaN / collapsed GPU output) is the Valhall
  // garbage signature, token-independent — always a hard failure.
