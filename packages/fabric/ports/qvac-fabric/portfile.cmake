vcpkg_from_github(
  ...
  REF v${VERSION}
  SHA512 7227c40cd4eb3a168e4d72219dffae3981753087268819ea87f9e65f5cf7144f2d984dfd7d6df60199d82332e5278c63a245a63f0a6cb93d1633fa6a40813ddd
  PATCHES
    # QVAC: mark the `common` data globals (build-info + log verbosity) with a
    # dllexport/dllimport marker so the inference addons can resolve them across
    # the single shared qvac-fabric runtime on Windows. No-op on ELF/Mach-O.
    # Tracked as an overlay port until upstreamed to the published port.
    patches/0001-qvac-common-data-api.patch
)

# QVAC: define QVAC_FABRIC_BUILD while compiling the runtime so the markers in
# the patched headers resolve to dllexport (the consumer build leaves it unset
# and gets dllimport). Applied via the flags vcpkg threads into CMAKE_*_FLAGS so
# it reaches every translation unit, including the generated build-info.cpp.
string(APPEND VCPKG_C_FLAGS " -DQVAC_FABRIC_BUILD")
string(APPEND VCPKG_CXX_FLAGS " -DQVAC_FABRIC_BUILD")
