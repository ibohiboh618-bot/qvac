...
function(detect_ndk_vulkan_version)
    ...
    cmake_path(SET ndk_home NORMALIZE "$ENV{ANDROID_NDK_HOME}")
    ...
    file(GLOB host_dirs LIST_DIRECTORIES true "${ndk_home}/toolchains/llvm/prebuilt/${host_system_name_lower}-*")
    if(host_dirs)
        list(GET host_dirs 0 host_dir)
        get_filename_component(host_arch "${host_dir}" NAME)
        set(vulkan_core_h "${ndk_home}/toolchains/llvm/prebuilt/${host_arch}/sysroot/usr/include/vulkan/vulkan_core.h")
    else()
    ...
    if(version_match)
        ...
        message(FATAL_ERROR "Could not extract VK_HEADER_VERSION from vulkan_core.h")
    endif()
    ...
    if(version_match)
        ...
        message(FATAL_ERROR "Could not extract major.minor version from vulkan_core.h")
    endif()
endfunction()
